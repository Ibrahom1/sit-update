...
# FIXED: Use ordered list instead of dictionary to maintain exact station order
STATION_ORDER = [
    # Ravi River stations (0-3)
    {"key": "Jassar", "api_name": "Jassar", "title": "Jassar at Ravi", "short_name": "Jassar"},
    {"key": "Shahdara", "api_name": "Shahdara", "title": "Shahdara at Ravi", "short_name": "Shahdara"},
    {"key": "Balloki", "api_name": "Balloki", "title": "Balloki at Ravi", "short_name": "Balloki"},
    {"key": "Sidhnai", "api_name": "Sidhnai", "title": "Sidhnai at Ravi", "short_name": "Sidhnai"},
    
    # Chenab River stations (4-6)
    {"key": "Marala", "api_name": "Marala", "title": "Marala at Chenab", "short_name": "Marala"},
    {"key": "Trimmu", "api_name": "Trimmu", "title": "Trimmu at Chenab", "short_name": "Trimmu"},
    {"key": "Panjnad", "api_name": "Panjnad", "title": "Panjnad at Chenab", "short_name": "Panjnad"},
    
    # Sutlej River stations (7-9)
    {"key": "Ganda Singh Wala", "api_name": "Ganda Singh Wala", "title": "Ganda Singh Wala at Sutlej", "short_name": "G. S. Wala"},
    {"key": "Sulemanki", "api_name": "Sulemanki", "title": "Sulemanki at Sutlej", "short_name": "Sulemanki"},
    {"key": "Islam", "api_name": "Islam", "title": "Islam at Sutlej", "short_name": "Islam"},
    
    # Indus River stations (10-12) - connected
    {"key": "Guddu", "api_name": "Guddu", "title": "Guddu at Indus", "short_name": "Guddu"},
    {"key": "Sukkur", "api_name": "Sukkur", "title": "Sukkur at Indus", "short_name": "Sukkur"},
    {"key": "Kotri", "api_name": "Kotri", "title": "Kotri at Indus", "short_name": "Kotri"},
]

# FIXED: Correct river groupings based on your requirements
# Each group contains the indices of stations that should be connected
GROUPS = [
...
def calculate_required_height(num_stations):
    ...
    # Add group gaps after Ravi (after station 3), Chenab (after station 6), Sutlej (after station 9), Indus (after station 12)
    # That's 4 group gaps total now
    num_group_gaps = 4
...
def create_dashboard(api_data):
    ...
    for i, row in enumerate(rows):
        print(f"  {i}: {row['short_name']} - {row['severity']}")
...
TEXT_W     = 690
TITLE_GAP  = 12
ROW_GAP    = 20    # Reduced from 24 to 20 to save space
GROUP_GAP  = 24    # Reduced from 28 to 24 to save space
HEADER_H   = 250
SEP_H      = 13
TIMEX      = W - 230
DOT_R      = 24
...
def generate_image(rows, date_text, time_text, outfile):
    ...
    for i, r in enumerate(rows):
        ...
        # After Ravi (station 3), Chenab (station 6), Sutlej (station 9), Indus (station 12)
        if i in (3, 6, 9, 12):  # After Sidhnai, Panjnad, Islam, Kotri
            y += GROUP_GAP
    ...
    # FIXED: Connectors with correct river groupings - now includes Indus River connections
    print("\n=== DRAWING CONNECTORS ===")
    ...
    for group_idx, group in enumerate(GROUPS):
        ...
        for a, b in zip(group, group[1:]):
            if a < len(dot_ys) and b < len(dot_ys):  # Safety check for dynamic stations
